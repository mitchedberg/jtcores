...
module jttoaplan1_main(
    ...
    // Sprite and Palette RAM read-back
...
always @* begin
    main_cs     = !ASn  && A[23:18] == 6'b000000;  // 0x000000-0x03FFFF (ROM 256KB)
    spr_cs      = !BUSn && A[23:12] == 12'b0000_0100_0000;  // 0x040000-0x040FFF (Sprite RAM)
    pal_cs      = !BUSn && A[23:12] == 12'b0000_0101_0000;  // 0x050000-0x050DFF (Palette)
    io_cs       = !BUSn && A[23:7]  == 17'b0000_0111_1000_000;  // 0x078000-0x0781FF (I/O)
    sndlatch_cs = !BUSn && A[23:1]  == 23'b0000_0111_1010_0000_0000_00 && !RnW;
    ram_cs      = !BUSn && A[23:14] == 10'b0000_0011_00;  // 0x030000-0x033FFF (RAM 16KB)
...
// 7 MHz clock enable from 48 MHz: num=7, den=48
jtframe_68kdtack_cen #(.W(5)) u_dtack(
    ...
    .DSn        ( {UDSn, LDSn}  ),
    .num        ( 4'd7          ),
    .den        ( 5'd6          ),  // 48/6 = 8 MHz, close to 7 MHz
    .DTACKn     ( DTACKn        ),
    .wait2      ( 1'b0          ),
    .wait3      ( 1'b0          ),
    .fave       (               ),
