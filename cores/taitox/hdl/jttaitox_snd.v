...
// Taito X Z80 + YM2610 sound module
// Z80 @ 4 MHz, clock enable generated from 48 MHz system clock

module jttaitox_snd(
    input             rst,
    input             clk,
    // Sound latch from main CPU (TC0140SYT stub)
    input      [ 7:0] snd_latch,
    input             snd_stb,
    // Z80 ROM (SDRAM) - fixed 32KB
...
always @(*) begin
    ...
    rom_cs  = !macc_n && !A[15];           // 0x0000-0x7FFF ROM (fixed 32KB)
    ram_cs  = !macc_n &&  A[15];           // 0x8000-0xFFFF RAM (2KB at 0x8000)
end

// --- I/O decode ---
always @(*) begin
    ym_cs    = !iorq_n && (!rd_n || !wr_n) && A[1:0] == 2'b00; // 0x00-0x03 YM2610
    latch_rd = !iorq_n && !rd_n && A[7:0] == 8'h04;            // 0x04 sound latch
end

// --- ROM address (fixed, no banking) ---
